...
function downrank(spell, key)
	...
	for i,v in pairs(tableofstuff[key]) do		--Get the index of the current spell in the table of values.
		...
		elseif hit and v ~= nil then			--from the index on...
			local spellid = getspellid(v)		--run comparisons to find the best answer.
...
function parsespell(spell, spellid)
	...
	for i in pairs(tableofstuff) do	
		if string.match(spell, 'ga') then
			if string.match(spell, i .. [[%s]]) then
				key = i														--Iterate through table
				return key
			end							-- not capable of handling the spell. Can't cast, end.
		elseif string.match(spell, i) then
			key = i														--Iterate through table
			return key
		end
	end
